...
/******/ 	return __webpack_require__(__webpack_require__.s = "./assets/sonata/js/panel.js");
/******/ })
/************************************************************************/
/******/ ({

/***/ "./assets/sonata/css/panel.css":
/*!*************************************!*\
  !*** ./assets/sonata/css/panel.css ***!
  \*************************************/
...
/* harmony import */ var _css_panel_css__WEBPACK_IMPORTED_MODULE_10__ = __webpack_require__(/*! ../css/panel.css */ "./assets/sonata/css/panel.css");
/* harmony import */ var _css_panel_css__WEBPACK_IMPORTED_MODULE_10___default = /*#__PURE__*/__webpack_require__.n(_css_panel_css__WEBPACK_IMPORTED_MODULE_10__);
...
$(document).ready(function () {
  $('[data-behavior="add-block"]').each(function () {
    $(this).on('change', $(this), function (e) {
      ...
      var selectedblockName = $addBlockSelect.find(':selected').data('name'); // Get the new index

      var index = $collectionHolder.data('index'); // Get the prototype data
      ...
      var newForm = proto.replace(/__umanit_block__/g, index); // HTML template

      var $ublock = $('<div class="ublock-body"></div>').append(newForm);
      var $embededItem = $('<div class="ublock ui-sortable-handle" data-block-type="' + selectedblockType + '" data-name="' + selectedblockName + '" data-order="' + index + '">' + '   <div class="ublock-header"><h3 class="ublock-title">' + NEW_BLOCK_STR + selectedblockName + '</h3>' + '       <div class="ublock-tools"><span title="' + NOT_SAVED_STR + '" class="ublock-label bg-yellow">' + selectedblockName + '</span>' + '           <button type="button" class="btn btn-ublock-tool" data-target="removeBoxItem"><i class="fa fa-trash"></i></button>' + '           <button type="button" class="btn btn-ublock-tool" data-collapse="ublock-body"><i class="fa fa-caret-up"></i></button>' + '       </div>' + '   </div>' + '</div>');
      $embededItem.append($ublock); // increment the index with one for the next item

      $collectionHolder.data('index', index + 1); // Display html content

      $collectionHolder.append($embededItem); // add custom javascript event on the new panel

      var eventItem = new CustomEvent('ublock.after_added', {
        'detail': $embededItem
      });
      document.dispatchEvent(eventItem); // Scroll to newly created block

      $('html, body').animate({
        scrollTop: $embededItem.offset().top - 130
      }, 800); // Reset select

      if ((typeof Select2 === "undefined" ? "undefined" : _typeof(Select2)) === 'object') {
        // Using jQuerySelect2
  ...
  $('.sortable').sortable({
    ...
    start: function start(event, ui) {
      var panel = ui.item.closest('.panel-holder')[0];
      var eventItem = new CustomEvent('ublock.on_sort_start', {
        detail: {
          panel: panel
        }
      });
    ...
    stop: function stop(event, ui) {
      ...
      var panel = $collectionHolder[0];
      var eventItem = new CustomEvent('ublock.on_sort_end', {
        detail: {
          panel: panel
        }
      });
